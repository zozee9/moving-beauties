...
float dt = .1; // time
float visc = 0.002; // viscosity
float diff = 0.00001; // diffusion rate
float source = 50; // how much density to add
float force = 13; // force from mouse movement for velocity
...
void draw_dens(int N, float[] dens) {
  ...
  for (i = 0; i <= N; i++) {
    ...
    for (j = 0; j <= N; j++) {
      ...
      float distance = dist(300, 300, x, y);
      if (distance<=200){
        beginShape();
        fill(239./255, 252./255, 138./255, d00);
        vertex(x,y);
        vertex(x+h,y);
        vertex(x+h,y+h);
        vertex(x,y+h);
...
void testMousePressed(float[] d, float[] u, float[] v) {
  if (mousePressed) {
    ...
    if (mouseButton == LEFT) {
      u[idx(i,j)] = force*(mouseX-pmouseX);
      v[idx(i,j)] = force*(mouseY-pmouseY);
    }
  }
}
...
void reset(float[] u, float[] v, float[] d) {
  ...
  //d[idx((N+2)/2,(N+2)/2)] = source;
}

void draw() {
  background(0);
  fill(211./255, 237./255, 235./255);
  ellipse(300, 300, 450, 450);
  rect(520, 285, 75, 50, 10, 10, 10, 10);
  fill(66./255, 55./255, 23./255);

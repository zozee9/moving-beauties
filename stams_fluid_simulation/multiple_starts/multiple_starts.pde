...
float Source_Strength = 1;
float Diffusion_Strength = 0.0005;
float Viscocity_Strength = 0;
...
void setup() {
  ...
  cp5.addSlider("Diffusion_Strength")
    ...
    .setRange(0, 0.0005)
  ;
  cp5.addSlider("Viscocity_Strength")
    .setPosition(20, 40)
    .setRange(0, 0.03)
  ;
...
void reset(float[] u, float[] v, float[] d) {
  ...
  d[idx((N+2)/2,N-2)] = Source_Strength;
  d[idx((N+2)/2 + 1,N-2)] = Source_Strength;
  d[idx((N+2)/2 - 1,N-2)] = Source_Strength;
  ...
  d[idx((N+2)/4,N-2)] = Source_Strength;
  d[idx((N+2)/4 + 1,N-2)] = Source_Strength;
  d[idx((N+2)/4 - 1,N-2)] = Source_Strength;
  ...
  u[idx((N+2)/4,N-2)] = -.5;
  u[idx((N+2)/4 + 1,N-2)] = -.25;
  u[idx((N+2)/4 - 1,N-2)] = -.25;
  
  d[idx(int((N+2)*(3./4)),N-2)] = Source_Strength;
  d[idx(int((N+2)*(3./4)) + 1,N-2)] = Source_Strength;
  d[idx(int((N+2)*(3./4)) - 1,N-2)] = Source_Strength;

  v[idx(int((N+2)*(3./4)),N-2)] = -2;
  v[idx(int((N+2)*(3./4)) + 1,N-2)] = -1;
  v[idx(int((N+2)*(3./4)) - 1,N-2)] = -1;
  
  u[idx(int((N+2)*(3./4)),N-2)] = .5;
  u[idx(int((N+2)*(3./4)) + 1,N-2)] = .25;
  u[idx(int((N+2)*(3./4)) - 1,N-2)] = .25;
}
...
void draw() {
  ...
  if (!paused) {
    ...
    vel_step(N,u,v,u_prev,v_prev,Viscocity_Strength,dt);

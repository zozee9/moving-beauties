...
float k = 1000; // force toward resting
float kv = 1500; // dampening
...
// sphere collision
float sphereRad = 300;
float[] spherePos = {700,0,zStart-100};
  ...
  void display() {
    ...
    for (int col = 0; col < getSize(-1)-1; col++) {
      for (int row = 0; row < getSize(col)-1; row++) {
        ...
        vertex((float)first[0],(float)first[1],(float)first[2],0,0); // top left
        vertex((float)second[0],(float)second[1],(float)second[2],tex.width,0); // top right
        vertex((float)third[0],(float)third[1],(float)third[2],tex.width,tex.height); // bottom right
        vertex((float)fourth[0],(float)fourth[1],(float)fourth[2],0,tex.height); // bottom left
        //noTint();
        endShape();
      }
    }
...
class Stitch {
  ...
  void updateAccVelPos(float dt) {
    ...
    collideSphere(this); // takes care of updating position already, which is real janky
  }
...
boolean collideSphere(Stitch s) {
  // i'm real lazy so i'm gonna cheat
  PVector spPos = new PVector(spherePos[0],spherePos[1],spherePos[2]);
  PVector stPos = new PVector((float)s.pos[0],(float)s.pos[1],(float)s.pos[2]);
  
  float distance = PVector.dist(spPos,stPos);
  float rad = sphereRad + s.rad;
  
  if (distance < rad) {
    PVector stVel = new PVector((float)s.vel[0],(float)s.vel[1],(float)s.vel[2]);
    PVector normal = (PVector.sub(stPos,spPos,null)).normalize(null);
    stPos = PVector.add(spPos, PVector.mult(normal,rad*1.01,null),null);
    PVector vnorm = PVector.mult(normal,PVector.dot(stVel, normal), null);
    stVel.sub(vnorm);
    stVel.mult(1);
    
    float[] pos = stPos.array();
    float[] vel = stVel.array();
    s.pos[0] = pos[0];
    s.pos[1] = pos[1];
    s.pos[2] = pos[2];
...
void drawLights() {
  ...
  pointLight(175,175,175,spherePos[0],spherePos[1],spherePos[2]);
}
...
void draw() {
  ...
  vertex(0,yStart,zStart);
  vertex(width,yStart,zStart-500);
  vertex(width,yStart-25,zStart-500);
  vertex(0,yStart-25,zStart);
  vertex(0,yStart,zStart);

    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                if (v.lifetime > .15 && v.lifetime < .95)
                {
                    float2 noiseUV = TRANSFORM_TEX(v.uv_rand.xy, _NoiseTex);
                    float2 random = v.uv_rand.zw;

                    float2 noiseUV1 = float2(noiseUV.x + _Time.y * _NoiseScroll.x + random.x, noiseUV.y + _Time.y * _NoiseScroll.y + random.y);
                    float2 noiseUV2 = float2(noiseUV.x + _Time.y * _NoiseScroll.x + random.x + 1, noiseUV.y + _Time.y * _NoiseScroll.y + random.y);
                    float2 noiseUV3 = float2(noiseUV.x + _Time.y * _NoiseScroll.x + random.x - 1, noiseUV.y + _Time.y * _NoiseScroll.y + random.y);
                    float2 noiseUV4 = float2(noiseUV.x + _Time.y * _NoiseScroll.x + random.x, noiseUV.y + _Time.y * _NoiseScroll.y + random.y + 1);
                    float2 noiseUV5 = float2(noiseUV.x + _Time.y * _NoiseScroll.x + random.x, noiseUV.y + _Time.y * _NoiseScroll.y + random.y - 1);

                    noiseUV = (noiseUV1 * 4 + noiseUV2 + noiseUV3 + noiseUV4 + noiseUV5) / 8;

                    float surfaceNoiseSample = tex2Dlod( _NoiseTex, float4(noiseUV, 0,0)).g;

                    // float noise = v.noise;
                    float displaceAmount = _Displacement * surfaceNoiseSample;
                    v.vertex.xyz += normalize(v.normal) * displaceAmount;// * displaceAmount;
                }
                else
                {
                    v.vertex.xyz += normalize(v.normal) * .0125; // offset a bit so it's not so drastic
                }

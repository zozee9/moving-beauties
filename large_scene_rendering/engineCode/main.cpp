...
int main(int argc, char *argv[]){
    ...
    UserCamera mainCamera  = UserCamera(
            getCameraPosFromLau(L), getCameraDirFromLau(L), getCameraUpFromLau(L),
            curScene.mainCam.FOV, screenWidth, screenHeight, nearPlane, farPlane
    );
    UserCamera debugCamera = UserCamera(
            getDebugCameraPosFromLau(L), getDebugCameraDirFromLau(L), getDebugCameraUpFromLau(L),
            curScene.mainCam.FOV, screenWidth, screenHeight, nearPlane, farPlane
    );
    ...
    while (!quit){
        ...
        mainCamera.updateBasis(
            getCameraPosFromLau(L),
            getCameraDirFromLau(L),
            getCameraUpFromLau(L)
        );
        debugCamera.updateBasis(
            getDebugCameraPosFromLau(L),
            getDebugCameraDirFromLau(L),
            getDebugCameraUpFromLau(L)
        );
        bool useDebugCamera = getDebugCameraState(L);
        ...
        if (useShadowMap && curScene.shadowLight.castShadow){
            ...
            lightPos = mainCamera.lookAtPoint - lightDir*lightDist; 
            lightUp = glm::cross(vec3(lightDir.y,lightDir.x,lightDir.z),mainCamera.lookAtPoint-lightPos);
            lightViewMatrix = glm::lookAt(lightPos,mainCamera.lookAtPoint, lightUp); 

            computeShadowDepthMap(lightViewMatrix, lightProjectionMatrix, curScene.toDraw, mainCamera);
        }
        ...
        mainCamera.updateViewFrustum();
        debugCamera.updateViewMatrix();

        mat4 activeView = useDebugCamera ? debugCamera.view : mainCamera.view;
        mat4 activeProj = useDebugCamera ? debugCamera.proj : mainCamera.proj;

        setPBRShaderUniforms(activeView, activeProj, lightViewMatrix, lightProjectionMatrix, useShadowMap);
        ...
        drawSceneGeometry(curScene.toDraw, mainCamera); //Pass 2A: Draw Scene Geometry

        // Draw camera frustum so the debug camera can see it
        if (useDebugCamera) {
            drawCameraFrustum(mainCamera, debugCamera);
        }
        ...
        drawSkybox(activeView, activeProj); //Pass 2C: Draw Skybox / Sky color
        ...
        ImGui::Text("Camera Pos %f %f %f",mainCamera.pos.x,mainCamera.pos.y,mainCamera.pos.z);
        ImGui::End();
...
void configEngine(string configFile, string configName){
    ...
    while( fgets(rawline,1024,fp) ) { //Assumes no line is longer than 1024 characters!
        ...
        if (commandStr == "useShadowMap"){ 
            ...
            farPlane = val;
            LOG_F(1,"Setting Far Plane to %f", farPlane);
        }

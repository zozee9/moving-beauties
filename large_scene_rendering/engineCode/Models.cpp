...
float findMaxRadiusInChildren(Model m) {
  float modelScale = std::max(m.transform[0][0], std::max(m.transform[1][1], m.transform[2][2]));

  // translation
  glm::vec3 origin = glm::vec3(0,0,0);
  glm::vec3 modelPos = glm::vec3(m.transform[3]); // model translation
  float dist = glm::length(origin - modelPos);
  LOG_F(1,"Dist for %s is %f",m.name.c_str(),dist);
  LOG_F(1,"rad alone for %s is %f, rad is %f, scale is %f",m.name.c_str(),m.originalRadius*modelScale,m.originalRadius,modelScale);

  float maxRadius = dist + m.originalRadius * modelScale;
  LOG_F(1,"Radius for %s is %f",m.name.c_str(),maxRadius);

  if (m.numChildren == 0) {
    return maxRadius;
  } else {
    for (int i = 0; i < m.numChildren; i++){
      // this took some mathing... not sure if it's right
      maxRadius = std::max(maxRadius, modelScale*findMaxRadiusInChildren(*m.childModel[i])+dist);
    }
    LOG_F(1,"max radius for %s is %f",m.name.c_str(),maxRadius);
...
void loadModel(string fileName){
  ...
  while( fgets(rawline,1024,fp) ) { //Assumes no line is longer than 1024 characters!
    ...
    if (commandStr.substr(0,1) == "["){ // "[" indicates new model
      ...
      models[curModelID].modelShell = false;
      LOG_F(INFO,"Creating PreFab Model with name: %s",modelName.c_str());
    }
    else if (commandStr == "lodLevel"){
      int space = line.find(" ")+1;
      int lodNameLength = line.find("Lod")-space;
      string lodStr = line.substr(space,lodNameLength);
      if (lodStr == "low") {
        models[curModelID].lodLevel = low;
      } else if (lodStr == "high") {
        models[curModelID].lodLevel = high;
      } else {
        LOG_F(WARNING,"WARNING. Unknown LOD: %s. LOD set to always.",commandStr.c_str());
      }
      LOG_F(1,"LOD of model is %s",lodStr.c_str());
    }
      ...
      LOG_F(1,"Translating by (%f, %f, %f)", tx, ty, tz);
    }
    else if (commandStr == "lowLodDist"){
      float lodDist;
      sscanf(rawline,"lowLodDist %f", &lodDist);
      models[curModelID].lowLodDist = lodDist;
      LOG_F(1,"Setting low lod to %f", lodDist);
    }
      ...
      models[childModelID].modelShell = false;
      ...
      // Find bounds and centroid of this model
      glm::vec3 vertexSum = glm::vec3(0);
      float maxDistFromOrigin = 0.0;

      // vertexData is stored as vx, vy, vz,  tx, ty,  nx, ny, nz (only need
      // vertex pos)
      for (int vertex = 0; vertex < vertexData.size(); vertex += 8) {
        glm::vec3 currentVert = glm::vec3(
            vertexData[vertex],
            vertexData[vertex + 1],
            vertexData[vertex + 2]
        );

        if (currentVert.x > maxDistFromOrigin) {
          maxDistFromOrigin = currentVert.x;
        }
        if (currentVert.y > maxDistFromOrigin) {
          maxDistFromOrigin = currentVert.y;
        }
        if (currentVert.z > maxDistFromOrigin) {
          maxDistFromOrigin = currentVert.z;
        }
      }

      // Create the bounding sphere
      models[childModelID].radius = maxDistFromOrigin;
      models[childModelID].originalRadius = maxDistFromOrigin;
  ...
  for (int i = 0; i < numModels; i++) {
    models[i].radius = findMaxRadiusInChildren(models[i]);
    LOG_F(1,"Radius for %s set to %f",models[i].name.c_str(),models[i].radius);
  }
}
